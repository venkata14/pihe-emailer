...
# NOTES
# df -> the dataframe for representatives staffors. Currently used.
# df_sen -> Fist senator CSV FILE used. Not used anymore
# df_senators -> Second CSV FILE used. Currently used.
...
# This is the Representative Staff directory from https://directory.house.gov/#/
# This is from the Google drive under "PIHE App/staffer_info/Representatives/MASTER"
gsheetid = "15-PxAZmJeknPHtWDyZau6k8gbhKBnZ1WRNMFRjb3oWk"
sheet_name = "Representative_Staffers"
...
# This is for the Second Senator CSV FILE. The original under "PIHE App/staffer_info/Senators/MASTER"
gsheetid = "1HwvqqayqUcZBc1_2sXetwJDsWW1-8MCqMvnY5xqPgDU"
sheet_name = "Senator_Staffers"
gsheet_url = "https://docs.google.com/spreadsheets/d/{}/gviz/tq?tqx=out:csv&sheet={}".format(gsheetid, sheet_name)
df_senators = pd.read_csv(gsheet_url)
# Cleaning the df. Removing Rows that do have nothing in the "Employer" columns
df_senators = df_senators[df_senators["Employer"].notnull()]
# Adding a "Name" column with LastName, FirstName
df_senators["Name"] = df_senators[['Last Name', 'First Name']].T.agg(', '.join)
...
try:
    ...
    st.error("There was an error retrieving the PIHE templates. Using backdrop.")
    templates = None
    use_templates = False
...
# Getting Senator + Representative Names
...
def get_mocs():
    if address:
        ...
        try:
            ...
            st.error('An Error occured when trying to identify your representatives. **Please use a more accurate Address. If that does not work, please use a different address.**')
            return [], [], [], False, None
    else:
        return [], [], [], False, None
...
if found_reps:
    with reps:
        ...
        with col1:
            ...
            # ====
            last_name = senetor1.split()[-1].capitalize()
            office_data = df_senators[df_senators["Employer"].str.contains(last_name)]
            st.info(''.join(["##### ", senetor1, '\n', 'Scheduler: Choose one down below', ]))
            # titles = office_data['Title'].copy()
            # Here I try to see if there is any scheduler titles, because if so, I will use thise
            scheduler = office_data["Title"].str.contains("chedul")
            number_results = scheduler.sum()
            if number_results == 0:
                show_info = office_data[['Name', 'Title', 'Email', 'Phone', "First Name", "Last Name"]]
                show_info.set_index('Name', inplace=True)
                
                with st.form('scheduler-senator1'):
                    ...
                    st.text("Email to: " + selected_index)
                    email_box_open= True
                    # Gathering and sending Data
                    staffer_email = show_info.loc[selected_index]["Email"]
                    s_first_name = show_info.loc[selected_index]["First Name"]
                    s_last_name = show_info.loc[selected_index]["Last Name"]
                    email_info_senetor1 = [staffer_email, 'senator1', last_name, s_first_name, s_last_name]
                    submitted = st.form_submit_button("Email")
                key_words = ['director','staff assistant', "chedul"]  
                ...
                show_rec = st.checkbox('Show Recommended', value=True, key="Senator 1 Show Recommended")
                show_all = st.checkbox('Show All', key="Senator 1 Show All")
                    
                # st.write('###### *Recommended Personal*')
                if show_rec:
                    st.dataframe(recommendations[['Title', 'Phone']])
                # st.write('###### *All Personal*')
                if show_all:
                    st.dataframe(show_info[['Title', 'Phone']])
                
            else:
                df_of_interest = office_data[scheduler]
                show_info = df_of_interest[['Name', 'Title', 'Email', 'Phone', "First Name", "Last Name"]]
                show_info.set_index('Name', inplace=True)
                with st.form('scheduler-senator1'):
        ...
        with col2:
            ...
            if number_results == 0:
                ...
                with st.form('scheduler-senator2'):
                    selected_index = st.selectbox('This MOC does not have a scheduler. You can instead email another staffer. We recommend you email a *Director* or a *Staff Assistant*', show_info.index)
                    st.text("Email to: " + selected_index)
                    email_box_open= True
                    # Gathering and sending Data
                    ...
                    s_first_name = show_info.loc[selected_index]["First Name"]
                    s_last_name = show_info.loc[selected_index]["Last Name"]
                    email_info_senetor2 = [staffer_email, 'senator2', last_name, s_first_name, s_last_name]
                    submitted = st.form_submit_button("Email")
                key_words = ['director','staff assistant', "chedul"]  
                regstr = '|'.join(key_words)
                bool_map_recs = show_info['Title'].copy()
                ...
                show_rec = st.checkbox('Show Recommended', value=True, key="Senator 2 Show Recommended")
                show_all = st.checkbox('Show All', key="Senator 2 Show All")
...
if email_box_open:
    with name_container:
        ...
        with col2:
            cc_email = st.text_input("Cc", placeholder="Cc", value="engage@pih.org")
